...
class AttendanceForm(BaseForm):
    """Form for StudentAttendance model"""
    ...
    def clean(self):
        ...
        # Check for duplicate StudentAttendance for same student, date, and session
        if date and student and session:
            ...
            if existing.exists():
                raise ValidationError(
                    f'StudentAttendance for {student} on {date} ({session}) already exists'
                )
...
class BulkAttendanceForm(forms.Form):
    """Form for bulk StudentAttendance entry"""
    ...
    session = forms.ChoiceField(
        choices=StudentAttendance.SESSION_CHOICES,
        widget=forms.Select(attrs={'class': 'form-control'}),
        required=True
    )
